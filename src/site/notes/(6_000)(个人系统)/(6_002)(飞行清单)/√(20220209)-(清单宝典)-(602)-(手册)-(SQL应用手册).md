...
## 05.数仓监控
1. 存储过程——20201005——(对象)删除日推送表——`p_pushclean`
...
26. 存储过程——20210316——报错作业记录查询——`pp_sysjob_failed`
27. 存储过程——20210320——表连带索引复制——`p_table_clone`
28. 存储过程——20210324——查阅登录账号数据库权限——`p_syslogin_check`
29. 存储过程——20210408——链接服务器详情查询——`p_sysservers_detail`
30. 存储过程——20210408——计划作业信息查询补充过程——`p_sysjob_other_detail`
31. 存储过程——20210419——搜索登录账号——`p_syslogin_name`
32. 存储过程——20210423——报错作业短信通知——`p_sysjob_sendsms`
33. 存储过程——20210427——报错作业OA通知——`p_sysjob_sendoa`
34. 存储过程——20220825——(信息)获取部门人员姓名拼音缩写——[[(2_021)(算法解析)/√(20220615)-(过程解析)-(000)-(数仓)-(P_STAFF_NAMEFORSHORT)|P_STAFF_NAMEFORSHORT]]
35. 存储过程——20220825——(权限)新建数据库账号复制已有账号权限——[[(2_021)(算法解析)/√(20220824)-(过程解析)-(000)-(数仓)-(复制权限-P_CLONE_NEWUSER)|P_CLONE_NEWUSER]]
...
+ 
+ 
+ 20210504——存储过程——`p_syscondition_space`——各数据库容量使用情况
+ 20210508——存储过程——`p_column_update`——灵活动态更新字段
+ 20210512——存储过程——`p_table_sync`——灵活配置批量更新报表数据
+ 20210515——存储过程——`xhq_syslink_revoke_proc`——链接服务器权限回收
+ 20210522——存储过程——`p_staff_nameforshort`——获取部门人员姓名缩写
+ 20210523——表值函数——`fx_JobStatus`——查询正在执行作业
+ 20210531——存储过程——`p_sysjobs_owner`——获取作业责任所有者
+ 20210531——存储过程——`p_allrun`——查询所有在跑脚本及作业
+ 20210531——存储过程——`p_TableSearch`——批量关键字搜索
+ 20210618——存储过程——`p_sysjob_history_failed`——作业执行失败历史清查
+ 20210622——存储过程——`p_KILL_BEFORE_RESTART`——重启前批量清理运行
+ 20210629——表值函数——`p_CUT`——定界符切割文本
+ 20210704——表值函数——`fx_KillComment`——剔除代码注释
+ 20210705——存储过程——`p_ProcCommentClean`——剔除过程注释
+ 20210719——存储过程——`p_sysjobstep_detail`——查询作业步骤信息
+ 20210730——存储过程——`P_TOOLBOX`——自编工具箱集成
+ 20210804——存储过程——`p_table_sync_T`——灵活配置批量更新报表数据[升级版]
+ 20210910——存储过程——`P_SYNCDATA_INSERT`——灵活动态插入数据
+ 20210913——存储过程——`P_XHEADGET`——获取表名全部字段名称
+ 20211020——存储过程——`p_CZS`——查询字段注释
+ 20211124——存储过程——`P_READC`——竖向并列查阅数据
+ 20211210——存储过程——`p_sysjobs_owner_clean_name`——作业命名分类清洗自动填充
+ 20220211——存储过程——`PS_CPU`——监控服务器概况
+ 20220215——存储过程——`PS_WAIT`——监控进程等待时间
+ 20220215——存储过程——`PS_BLOCK`——监控堵塞进程
+ 20220216——存储过程——`PS_NOT_ACTIVE`——监控读写停滞进程
+ 20220216——存储过程——`PS_HEAVY_QUERY`——监控最耗资源进程
+ 20220217——存储过程——`P_TABLE_SPACEUSED`——查询表格占用空间大小
