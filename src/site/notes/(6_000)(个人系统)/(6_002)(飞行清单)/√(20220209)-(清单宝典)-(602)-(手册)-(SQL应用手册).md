...
## 05.数仓监控
1. 存储过程——20201005——(<mark style="background: #064420A6;">对象</mark>)删除日推送表——`p_pushclean`
2. 存储过程——20201005——(对象)格式化脚本过程查询结果——`sp_helptext2`
3. 函数定义——20201005——(函数)<strong><font color=#FF0000>汉字转全拼音函数</font></strong>——`fn_getquanpin`
4. 函数定义——20201005——(清洗)字符串提取——`f_split`
5. 函数定义——20201005——(结构)获取列名——`HeadGet`
6. 存储过程——20201005——(字典)复制生产库栏位描述——`p_ReadZhuShi`
7. 存储过程——20201005——(字典)本地写入栏位描述——`p_writeZhuShi`
8. 存储过程——20201005——(作业)查询正在执行作业——`p_sysjob_running`
9. 存储过程——20201005——(作业)查询执行报错作业——`p_sysjob_failed`
10. 存储过程——20201005——(权限)链接服务器临时开通关闭——`p_syslink_onoff`
11. 存储过程——20201005——(权限)061开通登录账号——`p_syslogin_open`
12. 存储过程——20201005——(对象)辅助表格日期命名切割——`p_xhq_table_create_rename_support`
13. 存储过程——20201005——(权限)搜索链接服务器涉及的过程——`p_syslink_search_show_procedure`
14. 存储过程——20201005——(视图)日推表动态修改视图——`p_view_change`
15. 存储过程——20201005——(工具)复制过程脚本到目标数据库——`p_procedure_copy`
16. 存储过程——20201017——(对象)循环批量建表——`p_table_createnew`
17. 存储过程——20201105——(信息)生产链接服务器查询注释——`proc_commentFromSever`
18. 存储过程——20201105——(对象)<strong><font color=#FF0000>表列信息比较</font></strong>——`p_syscol_compare`
19. 存储过程——20201230——(函数)数值型日期计算后转换数值——`f_num_datetime_backto_num`
20. 存储过程——20201231——(结构)循环批量建立索引——`p_index_create`
21. 存储过程——20201231——(视图)<strong><font color=#FF0000>灵活动态视图更新</font></strong>——`p_view_change_new`
22. 存储过程——20201231——(结构)循环批量增删字段——`p_column_add_drop`
23. 存储过程——20201231——(函数)灵活空值修补过程——`p_NULL_bury`
24. 存储过程——20210219——(工具)<strong><font color=#FF0000>打印SELECT INTO语句</font></strong>——`p_TableCreatePrint`
25. 存储过程——20210309——(作业)计划作业信息查询——`p_sysjob_detail`
26. 存储过程——20210316——(作业)报错作业记录查询——`pp_sysjob_failed`
27. 存储过程——20210320——(对象)<strong><font color=#FF0000>表连带索引复制</font></strong>——`p_table_clone`
28. 存储过程——20210324——(权限)查阅登录账号数据库权限——`p_syslogin_check`
29. 存储过程——20210408——(信息)链接服务器详情查询——`p_sysservers_detail`
30. 存储过程——20210408——(作业)计划作业信息查询补充过程——`p_sysjob_other_detail`
31. 存储过程——20210419——(权限)搜索登录账号——`p_syslogin_name`
32. 存储过程——20210423——(作业)报错作业短信通知——`p_sysjob_sendsms`
33. 存储过程——20210427——(作业)报错作业OA通知——`p_sysjob_sendoa`
34. 存储过程——20210504——(对象)各数据库容量使用情况——`p_syscondition_space`
35. 存储过程——20210508——(工具)灵活动态更新字段——`p_column_update`
36. 存储过程——20210512——(工具)<strong><font color=#FF0000>灵活配置批量更新报表数据</font></strong>——`p_table_sync`
37. 存储过程——20210515——(权限)链接服务器权限回收——`xhq_syslink_revoke_proc`
38. 存储过程——20210522——(信息)获取部门人员姓名缩写——`p_staff_nameforshort`
39. 表值函数——20210523——(作业)查询正在执行作业——`fx_JobStatus`
40. 存储过程——20210531——(作业)获取作业责任所有者——`p_sysjobs_owner`
41. 存储过程——20210531——(监控)查询所有在跑脚本及作业——`p_allrun`
42. 存储过程——20210531——(信息)批量关键字搜索——`p_TableSearch`
43. 存储过程——20210618——(监控)作业执行失败历史清查——`p_sysjob_history_failed`
44. 存储过程——20210622——(监控)重启前批量清理运行——`p_KILL_BEFORE_RESTART`
45. 表值函数——20210629——(函数)<strong><font color=#FF0000>定界符切割文本</font></strong>——`p_CUT`
46. 表值函数——20210704——(工具)<strong><font color=#FF0000>剔除代码注释</font></strong>——`fx_KillComment`
47. 存储过程——20210705——(工具)剔除过程注释——`p_ProcCommentClean`
48. 存储过程——20210719——(作业)查询作业步骤信息——`p_sysjobstep_detail`
49. 存储过程——20210730——(信息)<strong><font color=#FF0000>自编工具箱集成</font></strong>——`P_TOOLBOX`
50. 存储过程——20210804——(工具)灵活配置批量更新报表数据[升级版]——`p_table_sync_T`
51. 存储过程——20210910——(工具)<strong><font color=#FF4500>灵活动态插入数据</font></strong>——`P_SYNCDATA_INSERT`
52. 存储过程——20210913——(对象)获取表名全部字段名称——`P_XHEADGET`
53. 存储过程——20211020——(字典)查询字段注释——`p_CZS`
54. 存储过程——20211124——(工具)竖向并列查阅数据——`P_READC`
55. 存储过程——20211210——(作业)作业命名分类清洗自动填充——`p_sysjobs_owner_clean_name`
56. 存储过程——20220211——(性能)监控服务器概况——`PS_CPU`
57. 存储过程——20220215——(性能)监控进程等待时间——`PS_WAIT`
58. 存储过程——20220215——(性能)监控堵塞进程——`PS_BLOCK`
59. 存储过程——20220216——(性能)监控读写停滞进程——`PS_NOT_ACTIVE`
60. 存储过程——20220216——(性能)监控最耗资源进程——`PS_HEAVY_QUERY`
61. 存储过程——20220217——(信息)查询表格占用空间大小——`P_TABLE_SPACEUSED`
62. 存储过程——20220825——(信息)获取部门人员姓名拼音缩写——[[(2_021)(算法解析)/√(20220615)-(过程解析)-(000)-(数仓)-(P_STAFF_NAMEFORSHORT)|P_STAFF_NAMEFORSHORT]]
63. 存储过程——20220825——(权限)新建数据库账号复制已有账号权限——[[(2_021)(算法解析)/√(20220824)-(过程解析)-(000)-(数仓)-(复制权限-P_CLONE_NEWUSER)|P_CLONE_NEWUSER]]
